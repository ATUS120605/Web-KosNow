...
for (let i = today.getDate(); i <= lastDayOfMonth; i++) {
   ...
   datesElement.innerHTML += `
    <div class="swiper-slide !w-fit py-[2px]">
        <label class="relative flex flex-col items-center justify-center w-fit rounded-3xl p-[14px_20px] gap-3 bg-white border border-white cursor-pointer transition-all duration-300 date-card">
            <img src="/assets/images/icons/calendar.svg" class="w-8 h-8" alt="icon">
            <p class="font-semibold text-nowrap">${date.getDate()} ${month}</p>
            <input type="radio" name="start_date" class="absolute top-1/2 left-1/2 -z-10 opacity-0 date-input" value="${realDate}" required>
        </label>
    </div>`;
}
