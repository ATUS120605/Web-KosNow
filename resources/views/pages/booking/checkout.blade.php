...
<body>
    ...
    <div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
        <a href="{{ url()->previous() }}"
            class="w-12 h-12 flex items-center justify-center shrink-0 rounded-full overflow-hidden bg-white shadow-sm">
            <img src="{{ asset('assets/images/icons/arrow-left.svg') }}" class="w-[28px] h-[28px]" alt="icon">
        </a>
        <p class="font-semibold text-[#141414]">Checkout Kos Atus</p>
        <div class="dummy-btn w-12"></div>
    </div>

    <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
        <div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white shadow-sm">
        ...
        <div
            ...
            <div class="flex flex-col gap-4 pt-[22px]">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <p class="font-semibold">{{\Carbon\Carbon::parse($transaction['start_date'])->format('d M Y')}}</p>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <img src="{{ asset('assets/images/icons/calendar.svg') }}" class="w-6 h-6 flex shrink-0" alt="icon">
                    ...
                    @php
                        $startDate = \Carbon\Carbon::createFromFormat('Y-m-d', $transaction['start_date']);
                        $endDate = $startDate->copy()->addMonths((int) $transaction['duration']);
                    @endphp

                    <p class="font-semibold">{{ $endDate->isoFormat('DD MMM YYYY') }}</p>
                </div>
            </div>
        </div>
            ...
            <div id="PaymentOptions" class="flex flex-col rounded-[30px] border border-[#F1F2F6] p-5 gap-4 mx-5">
                <div id="TabButton-Container"
                    ...
                    <label class="tab-link group relative flex flex-col justify-between gap-4"
                       ...
                       <div class="w-0 mx-auto group-has-[:checked]:ring-1 group-has-[:checked]:ring-[#00A3FF] group-has-[:checked]:w-[90%] transition-all duration-300">
                    </div>
                    </label>
    ...
    <div class="fixed bottom-5 w-full max-w-[640px] px-5 z-10">
        <div class="flex items-center justify-between rounded-[40px] py-4 px-6 bg-ngekos-black">
            <div class="flex flex-col gap-[2px]">
                <p id="price" class="font-bold text-xl leading-[30px]" style="color:#6ec8fc !important;">
                    </p>
                <span class="text-sm text-white">Grand Total</span>
            </div>
            <button type="submit"
                class="flex shrink-0 rounded-full py-[14px] px-5 font-bold text-white transition-all duration-300"
